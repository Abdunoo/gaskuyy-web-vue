...
<div class="relative flex flex-grow-0 w-screen h-screen">
   <div v-if="showMenu" class="flex-col hidden w-1/5 px-4 pt-4 space-y-8 bg-white lg:flex">
      <span class="text-2xl font-bold text-center text-blue-400">Gaskuyy</span>
      <div class="px-0 space-y-2 text-sm font-medium cursor-pointer">
         <div @click="dropdown = !dropdown" class="flex justify-between px-5 py-3 text-sm bg-white rounded-lg hover:text-white hover:bg-blue-400">
            <span>Home</span>
         </div>
         <div v-if="dropdown" class="pl-4 space-y-0">
            <div @click="category = ''" class="flex px-5 py-3 space-x-1 text-sm text-center rounded-lg">
               <span>All </span><span>Shoes</span>
            </div>
            <div @click="category = 'Outdoor'" class="px-5 py-3 text-sm rounded-lg">
               <span>Outdoor</span>
            </div>
            <div @click="category = 'Tennis'" class="px-5 py-3 text-sm rounded-lg">
               <span>Tennis</span>
            </div>
            <div @click="category = 'Running'" class="px-5 py-3 text-sm rounded-lg">
               <span>Running</span>
            </div>
         </div>
         <div @click="this.$router.push('/cart')" class="px-5 py-3 text-sm bg-white rounded-lg hover:text-white hover:bg-blue-400">
            <span>Keranjang</span>
         </div>
         <div v-if="role=='admin'" @click="this.$router.push('/admin')" class="px-5 py-3 text-sm bg-white rounded-lg hover:text-white hover:bg-blue-400">
            <span>Admin</span>
         </div>
         <div class="px-5 py-3 text-sm bg-white rounded-lg hover:text-white hover:bg-blue-400">
            <span>Profil</span>
         </div>
         <div @click="this.$router.push('/login')" class="px-5 py-3 text-sm bg-white rounded-lg hover:text-white hover:bg-blue-400">
            <span>Logout</span>
         </div>
      </div>
   </div>
      ...
      <div class="absolute inset-0 w-auto h-full px-4 overflow-auto bg-gray-100">
         <header class="flex flex-col py-4 space-y-4">
            ...
            <div class="flex items-center justify-between w-full mb-4 space-x-4">
               <div class="flex w-full space-x-1 text-base bg-white shadow rounded-xl">
                  <span class="py-3 pl-4 font-thin text-gray-400">
                     <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
                     </svg>
                  </span>
                  <input v-model="searchData" @input="onInputDebounced()" type="text" name="" id="" placeholder="looking for shoes" ondragenter="" class="flex-1 block w-full p-2 py-3 pr-4 rounded-l-md focus:outline-none rounded-xl" />
               </div>
               <div class="p-3.5 text-xl text-center text-white bg-blue-400 rounded-full">
                  <span>
                     <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
         ...
         <main class="space-y-4 overflow-hidden">
            <div class="relative flex flex-col flex-grow w-full h-20 space-y-6 lg:hidden">
               <span class="text-base font-medium text-gray-500">Select Category</span>
               <div class="absolute inset-0 flex items-center justify-start space-x-4 overflow-visible overflow-x-auto md">
                  <div @click="category = ''" class="flex px-5 py-3 space-x-1 text-sm text-center bg-white rounded-lg">
                     <span>All </span><span>Shoes</span>
                  </div>
            ...
            <div v-if="!category && !searchData" class="w-full space-y-2">
               ...
               <div v-if="!category && !searchData" class="flex md:space-x-4">
                  <div class="relative hidden w-full grid-cols-2 gap-2 bg-white rounded-lg md:grid">
                     ...
                     <div class="flex flex-col items-start justify-center">
                        ...
                        <span class="absolute bottom-0 right-0 -rotate-12"><img src="../components/assets/item4.png" alt="" class="" /></span>
                     </div>
                  </div>
                  <div class="relative grid w-full grid-cols-2 gap-2 bg-white rounded-lg">
                     ...
                     <div class="flex flex-col items-start justify-center">
                        <span class="font-sans text-xs font-semibold text-gray-500 -rotate-12 text-start"><span class="text-start">NEW!!!</span></span>
                        <span class="absolute bottom-0 right-0 -rotate-12"><img src="../components/assets/item3.png" alt="" class="" /></span>
                     </div>
                  </div>
               </div>
               <div class="w-full space-y-2">
            ...
            <div class="fixed bottom-0 left-0 right-0 flex justify-around w-full py-4 space-x-12 bg-white shadow md:max-w-full lg:hidden">
               <div class="flex justify-around w-1/2">
                  <svg @click="this.$router.push('/dashboard')" class="w-6 h-6 text-blue-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
                  ...
                  <svg v-if="role=='admin'" @click="this.$router.push('/admin')" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor" class="w-6 h-6 text-gray-600">
                     <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25zM6.75 12h.008v.008H6.75V12zm0 3h.008v.008H6.75V15zm0 3h.008v.008H6.75V18z" />
                  </svg>
               </div>
               <div class="flex justify-around w-1/2">
                  <svg @click="this.$router.push('/login')" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor" data-slot="icon" class="w-6 h-6 text-gray-600">
               ...
               <div class="w-full max-w-screen-xl p-4 py-6 mx-auto lg:py-8">
                  <div class="md:flex md:justify-between">
                     <div class="mb-6 md:mb-0">
                        <a href="https://sandbox2.panemu.com/abdunkuyy/" class="flex items-center">
                           <img class="h-8 me-3" alt="FlowBite Logo" src="https://sandbox2.panemu.com/abdunkuyy/assets/brand.png" />
                           <span class="self-center text-2xl font-semibold text-teal-400 whitespace-nowrap">Gaskuyy</span>
                        </a>
                     </div>
...
export default defineComponent({
   ...
   methods: {
      onInputDebounced: debounce(function () {
         this.searchQuery = this.searchData;
         this.performSearch(this.searchQuery);
      }, 500),

      async performSearch(newSearchValue) {
         if (newSearchValue != null || newSearchValue != undefined) {
            this.searchData = newSearchValue;
            this.getProduct();
         } 
      ...
      async getProduct() {
         let response = await makeRequest({
            url: `product?page=1&limit=100&search=${this.searchData}&category=${this.category}`,
            method: 'GET'
         })
