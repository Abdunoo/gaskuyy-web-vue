      ...
      <div class="absolute inset-0 p-4 overflow-auto">
         ...
         <div>
            <div class="mb-4">
               ...
               <input v-on:change="handleImageUpload"
            ...
            <div class="mb-4">
               ...
               <input v-model="product.title" type="text" class="w-full p-2 border border-gray-300 rounded"
                  placeholder="Name">
            </div>
            <div class="mb-4">
               <label class="block mb-2 text-sm font-medium text-gray-600">Price:</label>
               <input v-model="product.price" type="number" class="w-full p-2 border border-gray-300 rounded"
                  placeholder="price">
            </div>
            <div class="mb-4">
               <label class="block mb-2 text-sm font-medium text-gray-600">Discount:</label>
               <input v-model="product.discount" type="number" class="w-full p-2 border border-gray-300 rounded"
                  placeholder="discount">
            </div>
            <div class="mb-4">
               <label class="block mb-2 text-sm font-medium text-gray-600">Stock:</label>
               <input v-model="product.qty" type="number" class="w-full p-2 border border-gray-300 rounded"
                  placeholder="stock">
            </div>
            <div class="mb-4">
               <label class="block mb-2 text-sm font-medium text-gray-600">Category:</label>
               <select id="countries" v-model="product.category" :value="product.category"
            ...
            <button @click="saveProduk()"
               class="flex items-center justify-center p-2 mt-6 text-white bg-blue-600 rounded hover:bg-blue-700">
               <span>Save</span>
            </button>
   ...
   methods: {
      ...
      async saveProduk() {
         try {
            ...
            this.$toast.success('Produk berhasil disimpan');
            this.$emit('closeEdit');
         } catch (error) {
            this.$toast.error('Produk gagal disimpan');
            this.$emit('closeEdit');
         }
      },
      handleImageUpload(event) {
         ...
         if (!selectedFile || !selectedFile.type.match('image/*')) {
            this.$toast.error('Gambar yang dipilih tidak valid');
            return;
         }
